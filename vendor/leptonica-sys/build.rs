...
fn main() {
    ...
    // Trinity patch: libclang 的 freestanding 环境定义 C11 但无 atomics，
    // leptonica environ.h 走 atomic_int 分支失败（unknown type name）。
    // 强制 -std=c99 让 __STDC_VERSION__ < 201112 → 走 int fallback。
    bindings = bindings.clang_arg("-std=c99");
